...
// BUCKET SORT METHODS:
  ...
  void splitArrayToBuckets(struct Bucket* buckets, int* array, int threadId);
...
// MEMORY MANAGEMENT FUNCTIONS:
  void allocateBuckets(struct Bucket** buckets, int** bucketsOffsets);
  void freeBuckets(struct Bucket* buckets, int* bucketsOffsets);
...
int main(int argc, char* argv[])
{
  ...
  #pragma omp parallel num_threads(num_threads)
  {
    ...
    allocateBuckets(&buckets, &bucketsOffsets);
    ...
    #pragma omp single
    tmp = omp_get_wtime();
    splitArrayToBuckets(buckets, array, threadId);
    #pragma omp single
    ...
    freeBuckets(buckets, bucketsOffsets);
  }
...
void splitArrayToBuckets(struct Bucket* buckets, int* array, int threadId){
  ...
  int startIdx = (ARRAY_SIZE / num_threads) * threadId;
  for(i = 0, j = startIdx; i < ARRAY_SIZE; i++, j++){
    j %= ARRAY_SIZE;
    bucketId = ((double)array[j] / MAX_ELEM) * BUCKETS_NUM;
    if(bucketId % num_threads == threadId)
      buckets[bucketId].len++;
  }
  #pragma omp barrier
  for(i = threadId; i < BUCKETS_NUM; i += num_threads){
  ...
  for(i = 0, j = startIdx; i < ARRAY_SIZE; i++, j++){
    ...
    bucketId = ((double)array[j] / MAX_ELEM) * BUCKETS_NUM;
    if(bucketId % num_threads == threadId){
      buckets[bucketId].elems[buckets[bucketId].len] = array[j];
      buckets[bucketId].len++;
    }
  }
}
...
void printAndSaveResults(double tfillArray, double tSplitArray, double tMergeBuckets, double tSort, double tWriteBucketsToArray, double tWhole){
  ...
  FILE* results = fopen("resultsAlg1.txt", "w");
